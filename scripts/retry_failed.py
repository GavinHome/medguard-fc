"""补刷 checkpoint 中的失败 spec：原地重试 false 行，成功则替换。

设计:
  - specs 由固定种子生成，行号 ↔ spec 索引一一对应
  - 只重试 ok:false 的行；成功后原地替换为 {"ok": true, "traj": ...}
  - 行数不变 → build_train_set.py 的断点对齐不受影响；
    补刷后重跑 build_train_set.py 即可跳过全部 spec、直接重建训练文件

用法: uv run python scripts/retry_failed.py [--workers 8] [--max-rounds 2]
"""
import argparse
...
from build_train_set import build_plan, synth_one  # noqa: E402
...
def main():
    ...
    random.seed(42)                                       # 与主管线相同的 spec 序列
    ...
    llm = LLM()
    import re as _re
    norm = lambda q: _re.sub(r"[\s，。？！、；：\"'（）()?!,.:;]+", "", q).lower()
    seen = {norm(json.loads(l)["traj"]["query"])            # 已成功轨迹的 query 全部计入去重
    ...
    for round_no in range(1, args.max_rounds + 1):
        ...
        def work(i):
            return i, synth_one(llm, specs[i], tools, seeds, seen)
    ...
    print(f"补刷结束，剩余失败 {remain} 条（不影响 Gate，轨迹总数已达标）")
